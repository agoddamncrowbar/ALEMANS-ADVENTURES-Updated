...
import { rwandaParksData } from "./NationalParksInfo";
import type { ParkSection } from "./NationalParksInfo";

export default function RwandaParks() {
  const [activeIndex, setActiveIndex] = useState(0);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  const activePark = rwandaParksData[activeIndex];
    ...
    <div className="relative flex min-h-screen mt-14">
      ...
      >
        ...
        <ul className="space-y-1 pb-6">
          {rwandaParksData.map((park, index) => (
            <li
              key={park.id}
      ...
      <main className="flex-1 space-y-10 pb-10 px-4 md:px-4 md:ml-8 lg:ml-12 animate-fadeIn min-w-0 mt-10">
        ...
        <div className="relative">
          <h1 className="text-3xl uppercase tracking-wider font-light text-[#1A0A0B] px-4 py-2">
            Rwanda National Parks
          </h1>
        </div>
